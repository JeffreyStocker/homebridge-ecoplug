...
const _sendMessage = function(that, message, thisPlug, type, options = {}, callback) {
    const {retry_count = config.messageRetries, timeout = config.messageTimeout, port = config.defaultOutgoingPort} = options;
    let buffer;
    ...
    const timer = buffer.timer(id);
    if (!timer || retry_count >= 0) {
        socket.send(message, 0, message.length, port, thisPlug.host, function(err, bytes) {
            if (err && retry_count === 0) {
                callback({error: err, plug: thisPlug.id, message: "Error: Unable to send"}, null);
            } else {
                if (err) buffer.remove(id); 
                
                const timer = setTimeout(function () {
                    _sendMessage(that, message, thisPlug, {...options, retry_count: retry_count - 1}, callback);
                }.bind(this), timeout);
...
const handlePowerResponseMessage = function handlePowerResponseMessage(message, callback = () => {}) {
    ...
    powerMessageBuffer.remove(parsedMessage.id);
    callback(null, parsedMessage);
}
...
exports.sendStatusMessage = function sendsendStatusMessage(
    ...
    this._sendMessage(this, message, thisPlug, getOrSet, options, function(err, message) {
        if (err) {
            callback({error: err, plug: thisPlug.id, message: "Error: Unable to send"}, null);
        } else {
            if (callback) {
