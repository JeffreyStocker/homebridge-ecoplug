...
const _sendMessage = function(that, message, thisPlug, type, {retry_count = config.messageRetries, timeout = config.messageTimeout} = {}, callback) {
    let buffer;
    if (message.length === 130 || message.length === 128) {
...
const handleTimer = function handleTimer(message) {
    const type = message.type;
    if (type === 'set') {
        const buffer = statusMessageBuffer;
    } else if (type === 'get') {
        const buffer = powerMessageBuffer;
    } else if (type === 'discovery') {
        const buffer = discoveryMessageBuffer;
    } else {
        callback(`Error: Invalid type '${type}'`, null);
        return;
    }
